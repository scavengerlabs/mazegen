...
fn check_line_segments_close(first_segments: Vec<LineSegment>, second_segments: Vec<LineSegment>) {
    assert_eq!(first_segments.len(), second_segments.len());
    for (first_segment, second_segment) in first_segments.iter().zip(second_segments.iter()) {
